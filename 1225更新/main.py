...
def message_text(event):
    ...
    # DATABASE_URL = os.environ['DATABASE_URL']  # 雲端使用
    ...
    # 新增食物資料
    # 在任何新增食物的步驟中取消新增
    elif status in ["定義食物名稱", "定義單位", "定義熱量", "確認是否建立"] and text == "[取消建立]":
        def_search_food.cancel(line_bot_api, conn, event, user_id,
                               text, status)
    # 定義要建立的食物名稱
    elif status == "定義食物名稱":
    ...
    # 定義要建立的食物熱量
        ...
        def_add_food.food_kal(
            line_bot_api, conn, event, user_id, text, status)
    # 確認是否建立食物資料
    elif status == "確認是否建立" and text in ["[建立食物並扣除]", "[建立食物]"]:
